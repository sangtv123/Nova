 ...
 */
export function deserializeProps(serialized) {
    const data = JSON.parse(serialized);
    const props = {};
    for (const [key, value] of Object.entries(data)) {
        if (value && typeof value === 'object' && value.__type === 'Date') {
            props[key] = new Date(value.value);
        }
        else {
            props[key] = value;
        }
 ...
 */
function buildHydrationData(el) {
...
export async function mountIslands() {
    ...
    for (const el of markers) {
        ...
        if (strategy === 'eager')
            ...
            visible.push(el); // default: 'visible'
    }
 ...
 */
function scheduleVisibleHydration(elements) {
 ...
 */
function scheduleIdleHydration(elements) {
 ...
 */
export function extractIslandMetadata(html) {
    const islands = [];
 ...
 */
export function generateIslandPlaceholder(id, name, props, strategy = 'visible') {
    const hydrationData = serializeProps(props);
    return (`<div data-nova-island="${id}" ` +
