...
const transpileCache = new Map<string, { mtime: number, code: string }>();

// Persistent esbuild contexts for @nova/* framework packages (bundled once, kept alive)
const esbuildContexts = new Map<string, any>();

async function incrementalBuild(entryPoint: string, buildOptions: any): Promise<string> {
  ...
  const handleRequest = async (req: any, res: any) => {
    ...
    try {
      ...
      if (!fs.existsSync(fullPath) || !fs.statSync(fullPath).isFile()) {
        if (fs.existsSync(fullPath + '.tsx')) fullPath += '.tsx';
        else if (fs.existsSync(fullPath + '.ts')) fullPath += '.ts';
        else if (fs.existsSync(fullPath + '/index.tsx')) fullPath += '/index.tsx';
      ...
      if (fullPath.match(/\.(tsx?|ts)$/) || requestUrl === '/src/main.tsx') {
        if (fs.existsSync(fullPath)) {
          ...
          if (isMain) {
            ...
            const customPipes: string[] = config.customPipes || [];
            const routeCode = pages.map(p => `router.registerRoute('${p.path}', () => import('${p.importPath}'), [${p.layouts.map((l: string) => `() => import('${l}')`).join(', ')}]);`).join('\n');
            const islandCode = islands.map((i: any) => `registerIsland('${i.name}', () => import('${i.importPath}'));`).join('\n');
            ...
            const ensureImport = (pkg: string, id: string) => {
              const m = source.match(new RegExp(`import\\s+\\{([^}]*)\\}\\s+from\\s+['"]${pkg.replace('/', '\\/')}['"]`));
              if (m) { const ids = m[1].split(',').map((s: string) => s.trim()); if (!ids.includes(id)) source = source.replace(m[0], `import { ${[...ids, id].join(', ')} } from '${pkg}'`); }
              else if (!source.includes(`'${pkg}'`) && !source.includes(`"${pkg}"`)) source = `import { ${id} } from '${pkg}';\n${source}`;
            };
            ensureImport('@nova/router', 'router');
            ensureImport('@nova/islands', 'registerIsland');
            if (customPipes.length > 0) { ensureImport('@nova/signals', 'registerPipe'); source = `${pipeImports}\n${source}`; }
            const injection = `\n// Auto-routing\n${routeCode}\n// Auto-islands\n${islandCode}\n// Auto-pipes\n${pipeCode}`;
          ...
          const customPipesAll: string[] = config.customPipes || [];
...
export function createNovaPlugin(pluginManager: PluginManager, ctx: PluginContext) {
  return {
    ...
    setup(build: any) {
      ...
      build.onResolve({ filter: /\.scss(\?inline)?$/ }, (args: any) => {
      ...
      build.onLoad({ filter: /.*/, namespace: 'scss-raw' }, async (args: any) => {
        try {
