...
function warnLargeChunk(fileName: string, sizeBytes: number, limitKB = 50): void {
  if (sizeBytes > limitKB * 1024) {
    console.warn(
      `⚠️  ${fileName} is ${fmtBytes(sizeBytes)} — exceeds ${limitKB} kB limit.\n` +
...
export class Builder {
   ...
   */
  async buildMain(): Promise<BuildResult> {
    const entryPoints =
    ...
    if (result.metafile) {
      for (const [file, info] of Object.entries(result.metafile.outputs)) {
        warnLargeChunk(file, (info as any).bytes);
      }
    }
    ...
    return {
      ...
      modules: result.metafile
        ? Object.fromEntries(
            Object.entries(result.metafile.outputs).map(([k, v]) => [k, fmtBytes((v as any).bytes)])
          )
        : {},
    };
  }
   ...
   */
  async buildIslands(islands: Array<{ id: string; entry: string }>): Promise<IslandBundle[]> {
    ...
    for (const island of islands) {
      ...
      const sizeBytes = (outputInfo as any)?.bytes ?? 0;
   ...
   */
  async minify(code: string): Promise<string> {
   ...
   */
  async buildSSR(entry: string): Promise<BuildResult> {
    const result = await build({
      entryPoints: [entry],
      bundle: true,
      format: 'esm',
   ...
   */
  async printAnalysis(metafile: any): Promise<void> {
   ...
   */
  async saveAnalysis(metafile: any, outPath = 'dist/bundle-analysis.json'): Promise<void> {
 ...
 */
export function createBuilder(options: BuildOptions): Builder {
